class Api::Client::DashboardController < Api::Client::BaseController
  def index
    current_orders = current_user.future_arts.where(status: [:new, :accepted, :in_progress])
    ...
    stats = {
      ...
      active_orders: current_user.future_arts.where(status: [:accepted, :in_progress]).count,
