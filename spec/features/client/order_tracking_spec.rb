...
RSpec.describe 'Client Order Tracking', :js, type: :feature do
  let(:admin) { create(:admin_user) }
  let(:client) { create(:client_user) }
  let!(:active_pack) { create(:cycle_pack, :with_cycles, user: admin) }

  let!(:active_order) do
    create(:future_art, :commission, :in_progress,
           client: client, user: admin, cycle_pack: active_pack)
  end
  let!(:completed_order) do
    create(:future_art, :commission, :completed,
           client: client, user: admin)
  end
  let!(:pending_order) do
    create(:future_art, :commission,
           client: client, user: admin)
  end
