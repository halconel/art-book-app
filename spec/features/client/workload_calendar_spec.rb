...
RSpec.describe 'Client Workload Calendar', :js, type: :feature do
  let(:admin) { create(:admin_user) }
  let(:client) { create(:client_user) }

  # Create historical data for the calendar
  let!(:current_pack) { create(:cycle_pack, :with_cycles, user: admin, current_cycles: 8) }
  let!(:completed_pack1) do
    create(:cycle_pack, :completed, user: admin,
                                    started_at: 2.months.ago,
                                    completed_at: 1.month.ago)
  end
  let!(:completed_pack2) do
    create(:cycle_pack, :completed, user: admin,
  ...
  describe 'Queue integration and planning tools' do
    scenario 'Client sees current order queue context in calendar', :aggregate_failures do
      ...
      create(:future_art, :commission, client: client, user: admin, status: 'ordered')
      create_list(:future_art, :commission, user: admin, status: 'ordered', estimated_hours: 20)
