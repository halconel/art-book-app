...
RSpec.describe 'Order Acceptance Workflow', type: :feature, js: true do
  let(:admin) { create(:admin_user) }
  let(:client) { create(:client_user) }
  let!(:active_pack) { create(:creative_pack, user: admin) }
  ...
  describe 'Order status management' do
    ...
    scenario 'Admin starts work on commissioned order' do
      ...
      expect(commission.creative_pack).to eq(active_pack)
    ...
    scenario 'Admin marks commission as completed with final artwork' do
      commission.update!(status: 'in_progress', creative_pack: active_pack)
